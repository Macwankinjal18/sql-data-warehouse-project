...
BEGIN
	...
	REPLACE(cid,'-','') AS cid,
	   CASE
			WHEN UPPER(TRIM(REPLACE(REPLACE(REPLACE(cntry, CHAR(9), ''), CHAR(10), ''), CHAR(13), ''))) IN ('USA','US')
				THEN 'United States'

			WHEN UPPER(TRIM(REPLACE(REPLACE(REPLACE(cntry, CHAR(9), ''), CHAR(10), ''), CHAR(13), ''))) = 'DE'
				THEN 'Germany'

			WHEN cntry IS NULL 
				 OR TRIM(REPLACE(REPLACE(REPLACE(cntry, CHAR(9), ''), CHAR(10), ''), CHAR(13), '')) = ''
				THEN 'n/a'
	...
	PRINT '>> ===========================';
	PRINT '>> Loading Silver layer is Completed: ' + CAST(DATEDIFF(Second,@start_batchtime, @end_batchtime) AS NVARCHAR) + 'seconds';
	PRINT '>> ===========================';
